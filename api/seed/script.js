...
const reviewsToCompaniesData = fs.readFileSync('./data-join-companies-reviews', 'utf8');
...
companiesData.split("\n").forEach(v => {
  ...
  try {
    ...
    if(!COMPANY_REVIEWS[companyId] || !COMPANY_JOBS[companyId] || COMPANY_JOBS[companyId].length === 0) {
...
jobsData.split("\n").forEach(v => {
  ...
  try {
    ...
    if(!JOB_COMPANY[jobId] || !JOB_REVIEWS[jobId] || JOB_REVIEWS[jobId].length === 0) {
...
reviewsData.split("\n").forEach(v => {
  try {
    ...
    if(noContents(reviewId) || noContents(reviewCreatedAt) || noContents(reviewUpdatedAt) || noContents(reviewBody) || noContents(reviewSalaryAmt) || noContents(reviewSalaryCurrency) || noContents(reviewSalaryPeriod) || noContents(overallRating) || noContents(learningMentorshipRating) || noContents(meaningfulWorkRating) || noContents(workLifeBalanceRating) || noContents(isSpam) || noContents(isLegacy) || reviewSalaryCurrency.length !== 3) {
    ...
    if(Boolean(parseInt(isSpam))) {
      SPAM_REVIEWS[reviewId] = {original, reviewId, reviewCreatedAt, reviewUpdatedAt, reviewDeletedAt, reviewBody: reviewBody.replaceAll("\\'", "'").replaceAll('\\"', '"').replaceAll("\\n", "\n"), reviewSalaryAmt: parseFloat(reviewSalaryAmt), reviewSalaryCurrency, reviewSalaryPeriod, overallScore: parseFloat(overallRating), learningMentorshipScore: parseFloat(learningMentorshipRating), meaningfulWorkScore: parseFloat(meaningfulWorkRating), workLifeBalanceScore: parseFloat(workLifeBalanceRating), reviewTags: !!reviewTags ? reviewTags.split(",") : reviewTags, isSpam: Boolean(parseInt(isSpam)), isLegacy: Boolean(parseInt(isLegacy)), rest};
    } else if(!REVIEW_COMPANY[reviewId]) {
      ORPHAN_REVIEWS[reviewId] = {original, reviewId, reviewCreatedAt, reviewUpdatedAt, reviewDeletedAt, reviewBody: reviewBody.replaceAll("\\'", "'").replaceAll('\\"', '"').replaceAll("\\n", "\n"), reviewSalaryAmt: parseFloat(reviewSalaryAmt), reviewSalaryCurrency, reviewSalaryPeriod, overallScore: parseFloat(overallRating), learningMentorshipScore: parseFloat(learningMentorshipRating), meaningfulWorkScore: parseFloat(meaningfulWorkRating), workLifeBalanceScore: parseFloat(workLifeBalanceRating), reviewTags: !!reviewTags ? reviewTags.split(",") : reviewTags, isSpam: Boolean(parseInt(isSpam)), isLegacy: Boolean(parseInt(isLegacy)), rest};
    } else {
      const factor = reviewSalaryPeriod === 'hourly' ? 1 : reviewSalaryPeriod === 'weekly' ? 40 : reviewSalaryPeriod === 'monthly' ? 160 : 1;
      ...
      REVIEWS[reviewId] = {
        ...
        reviewUpdatedAt, 
        reviewDeletedAt, 
        reviewBody: reviewBody.replaceAll("\\'", "'").replaceAll('\\"', '"').replaceAll("\\n", "\n"), 
        ...
        workLifeBalanceScore: parseFloat(workLifeBalanceRating), reviewTags: !!reviewTags ? reviewTags.split(",") : reviewTags, 
...
const companiesWriteData = Object.values(COMPANIES).map(company => {
  ...
  if(!companyJobIds || companyJobIds.length === 0 || !companyReviewIds || companyReviewIds.length === 0) {
  ...
  return ({
    ...
    scoreTotals: {
      overall: companyReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].overallScore;
      }, 0),
      learningMentorship: companyReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].learningMentorshipScore;
      }, 0),
      meaningfulWork: companyReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].meaningfulWorkScore;
      }, 0),
      workLifeBalance:  companyReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].workLifeBalanceScore;
      }, 0),
    },
...
const jobsWriteData = Object.values(JOBS).map(job => {
  ...
  const currencies = new Set(jobReviewIds.map(id => REVIEWS[id].reviewSalaryCurrency));
  if(currencies.size > 1) {
    console.log("job with multiple salaries detected uh oh", job.jobId, job.jobName, job.location, COMPANIES[correspondingCompanyId].companyName);
  } 
  if(![...currencies][0]) {
  ...
  return ({
    ...
    scoreTotals: {
      overall: jobReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].overallScore;
      }, 0),
      learningMentorship: jobReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].learningMentorshipScore;
      }, 0),
      meaningfulWork: jobReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].meaningfulWorkScore;
      }, 0),
      workLifeBalance:  jobReviewIds.reduce((acc, curReviewId) => {
        return acc + REVIEWS[curReviewId].workLifeBalanceScore;
      }, 0),
    },
    salaryMin: {
...
const companiesFirebaseWriteData = companiesWriteData.map(({ logo, jobIds, reviewIds, createdAt, updatedAt, ...rest }) => ({ 
  logoRef: logo, 
...
const jobsFirebaseWriteData = jobsWriteData.map(({ companyId, reviewIds, createdAt, updatedAt, ...rest }) => ({ 
...
const reviewsFirebaseWriteData = reviewsWriteData.map(({ companyId, jobId, createdAt, updatedAt, ...rest }) => ({ 
  companyRef: db.collection("companies").doc(companyId), 
...
fs.writeFileSync('./data.json', JSON.stringify({ companies: companiesWriteData, jobs: jobsWriteData, reviews: reviewsWriteData }));
...
jobsFirebaseWriteData.forEach(({ id, ...rest }) => {
  db.collection("jobs").doc(id).set(rest)
  .then(() => {
