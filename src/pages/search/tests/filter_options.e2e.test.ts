describe("Filter options", () => {
  it("should be able to filter by type", () => {
    ...
    cy.get(".review-card").its("length").should("be.gt", 0);
  });

  it("should be able to filter by min and/or max salary range", () => {
    cy.visit("/reviews");
    cy.get(".review-card").first().contains("Labadie - Kunde").should("exist");
  ...
  it("should be able to filter by min and/or max rating range", () => {
    ...
    cy.get(".job-card").eq(1).contains("2.0").should("exist");
    ...
    cy.get(".job-card").eq(1).should("not.exist");
